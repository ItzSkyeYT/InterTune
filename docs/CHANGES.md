...
Not all of this ships at once. 0.10.8 is a small release cut from this same branch, so the engine as a Quick picks source, Try both, the context chips, the long-press card controls and the widget are held behind one compile-time switch (`constants/Unreleased.kt`) and two `android:enabled="false"` attributes, rather than on a branch of their own that would need every fix twice. What does ship in 0.10.8 is the listening log and its ledger, because the engine needs a history to arrive to, and because a log nobody can see is not one anybody should accept. Five of the explanation texts have a second, shorter version for that release, since the full ones name sources 0.10.8 does not offer.
