...
The Quick picks row can now come from InterTune itself, built on the phone from how the listener actually listens, and it learns. Upstream's row is either YouTube's own shelf or a query over `related_song_map` seeded by the last five plays, the week's most played and the last ten likes; neither knows whether a play was finished or skipped, searched for or autoplayed, nor whether a card was ever seen. The query also had two defects, both inherited: it counted a seed twice when two seeds shared a candidate, and Forgotten favourites could surface a song still in daily rotation. Both are fixed and covered by tests that run against the exported Room schema.

Every play is now a row in `listen`, opened when sound starts (not when the item loads), checkpointed every minute and at every pause, closed by its own playback stats or as stopped at the next launch if the process died, with how it ended, where it began (search, playlist, album, artist, a Home row and its slot, radio, library, history, an outside controller, a resumption), how deep into an autoplay chain it was, its session (thirty minutes of silence starts a new one) and the context chip it played under. A resume of the same song within five seconds of where a stop left off, inside a day, points at the stopped row, so a pause and a closed app is one listen. Seeks, repeat one, volume changes, playlist adds, downloads, shares, lyrics, artist pages and queue removals are recorded as signals. The old `event` log is converted once, with the clock corrected for the way `Converters` stored a `LocalDateTime` as though the wall clock were UTC.

The engine (`engine/`, plain Kotlin over plain rows, so the JVM tests run the shipped code) turns each listen into a value (engagement from time played, a skip that counts most near the middle, intent from how it started), folds the log once per build into per-song and per-artist numbers, and scores candidates on twelve features with eighteen weights: activation with ACT-R's recency, satiation, a growing gap, dormancy, a like, referral by the build's seeds, the artist's strength, novelty, cards passed over, co-occurrence with the session's artists, fit to the day part or the mood, and an artist crowding the week. Sixteen seeds from five pools, five lanes (related, again, artist, rediscover, explore) under fixed quotas, one card per version group, at most two per artist, three related per seed, half of each lane sampled at temperature one. Cards carry the two largest terms of their score as a caption. Rows are stored, so Home shows the last build at once; shadow builds run unseen under the other sources and are scored a day later against what was played.

Learning: every shown card is an impression; a tap is matched to its listen by the moment of the tap; a card played grades by engagement, one played elsewhere within a day at half, one passed over at 0.3; the weights step by the error inside their bounds, pulled slowly to their priors, a penalty never turning positive, at most one unit of movement a day; a pick from a build's spare pool teaches pairwise. Everything is on a switch: the source (YouTube, library, Best recommendations, Try both, Off), Rank with your listening and Tidy Home rows (the only two on by default), Show reasons, Adventurousness, Familiarity, New songs only, context chips, exclusions with expiry and Undo, Rest songs I skip, Learn from listening, Forget a session or a day, Reset, Rebuild, Export, and a developer page with thirty constants and the collision report.

YouTube's own shelf is chosen by name and by content, not by shape alone. The feed arrives in batches, several of its carousels are lists of songs to the parser, and the row was filled from whichever came last: a signed-in feed that ran Covers and remixes, Quick picks, Trending songs for you, Long listens put twenty hour-long mixes under the Quick picks heading. `QuickPicksShelf` takes the carousel whose title is the app's own localised word for Quick picks, else the first list of songs at least six in ten of which are under ten minutes, and a shelf found by title is never displaced by a later batch; a shelf found by content yields only to one found by title. Every song carousel is logged with its share of songs, so the choice can be read off logcat.

A pull to refresh brings songs that were not there before. Each row remembers what it showed in its last three fillings and puts that at the back when it is filled again (`RecentlyShown`), so the classic query's row is the other half of its pool rather than the same twenty shuffled, and Forgotten favourites and Keep listening draw from twice a row's worth. The YouTube row's pool has three tiers: the shelf, then the feed's other song shelves, then what YouTube lists as related to two of the shelf's songs, fetched on each pull and kept up to a hundred across loads. The shelf itself is the same for a whole session, so without the other two tiers a refresh could only ever reorder it. Verified on the emulator signed out, where the feed is an eight-song shelf and nothing else: three pulls gave three rows of twenty with no song in common. Between refreshes the rows hold still: the just-played list the tidy pass filters with is taken once per load, so a card played from the row stays where it is, playing, until the next pull, rather than vanishing while the listener is away in the player.
...
Every item on the Recommendations screen carries a small "i" with the long version behind it: the mechanism, the data it reads, the default and the caveat. The thirty four texts were written against the implementation and then checked back to it by a reader whose only job was to refute them, which killed two thirds of the first drafts. Rank with your listening does not merely defer to the engine, it is skipped outright when the engine built the row; the just-played filter wants thirty seconds as well as 45 percent of the length; Discover ignores the Adventurousness dial entirely; the three moods do nothing at all until twenty plays have been tagged under them. The texts say those things because the code does them.

Not all of this ships at once. 0.10.8 is a small release cut from this same branch, so the engine as a Quick picks source, Try both, the context chips, the long-press card controls, the whole Recommendations settings screen and the widget are held behind one compile-time switch (`constants/Unreleased.kt`) and two `android:enabled="false"` attributes, rather than on a branch of their own that would need every fix twice. The listening log itself does ship, silently: nothing reads it and nothing shows it, but 0.11 then arrives to a history rather than to nothing, and Privacy and history, which ships, is where it is paused and cleared. Two of the explanation texts have a second, shorter version for that release, because Quick picks source is the one setting left that would otherwise name a source 0.10.8 does not offer.

Holding the screen also holds the off switches for Tidy Home rows and Rank with your listening, which live nowhere else. Both stay on in 0.10.8, which is their default and the behaviour the release is announced with.

## Backups, the queue, and playlists as files

Three things a music player should have had already, all opt in, none of them touching what the app does today.

**Automatic backups.** The manual backup writes a zip of the settings file and the database; `BackupWriter` now owns that format so one writer serves both paths, and `BackupRestoreViewModel.backup` delegates to it unchanged. `AutoBackupWorker` writes the same file into a folder the listener picked, through the storage access framework so no permission beyond the one grant is needed, then prunes to the Keep setting, which is a slider from one to twenty rather than three fixed choices, since how many backups are worth keeping depends on the size of the library and the space to hand. The number above it follows the finger and is written when the finger lifts, so a drag costs one write rather than twenty and the line never trails the thumb. `AutoBackupPolicy` is the pure half and the tested half: it names files exactly as the manual path does, recognises only files this app wrote and only under this app's name, and picks the oldest by the timestamp inside the name rather than by whatever order the folder lists. Everything else in the folder is left alone, which matters because people back up into folders that hold other things. A write that fails part way deletes its own half-written file rather than leaving something Restore would accept. Nothing retries forever: a folder that has gone away is reported on the settings screen and the run ends.

**The queue as a playlist.** A button in the queue sheet turns what is playing into a local playlist, in the order shown, named by the listener, with a default name that avoids colliding with a playlist they already have. Radio queues hold songs the library has never seen, so the save inserts those into the song table inside the same transaction that writes the playlist; a playlist row that pointed at a song that did not exist would be a broken playlist.

**Playlists as M3U.** The per-playlist export existed inside two menus with the text built inline in each. `M3u` is that builder, extracted and tested byte for byte against what those menus wrote, plus file naming that survives FAT and Android: illegal characters replaced, runs of spaces collapsed, collisions numbered. Settings then gained one action that writes every library playlist into a folder in one go.
...
Each widget is configured on its own, when it is dropped and again from the launcher's edit button, and nothing it chooses touches the app's settings: two widgets side by side can be a player in the cover's own colour and a plain list of forgotten favourites. What it shows (what's playing, a list, or both), which list (Quick picks, Forgotten favourites, Keep listening, Recently played), how many rows at most, the background (the system's, dark, light, taken from the artwork, or none at all) and how solid it is, which buttons (previous, play and next, play alone, or none), the text size, whether artwork, artist names and the heading are drawn, and whether the corners are rounded. Everything falls back when it was never chosen, and a value from a later version reads as the default rather than breaking the widget.

The shape follows the size rather than scaling: one cell is the cover with a play button over it; a line of song and its buttons at the usual four by two, with rows underneath; narrow and tall stacks the cover above the song; given only what's playing and enough room, the cover takes the whole widget. Recently played is kept by the widget itself as songs come and go rather than by asking the database, since the song that just started is the newest there is.
