 ...
 *
 * It lived inline on the Advanced screen and was 420 of that file's 701 lines, which made the
 * handful of rows an ordinary person might actually want impossible to find among them. Nothing
 * here has changed except where it lives.
 *
 * Note this block is deliberately edge to edge: the colour swatches are fillMaxWidth backgrounds
 * meant to bleed, so the caller must not wrap it in a card or a gutter.
 */
@Composable
fun ColumnScope.DeveloperFrag(navController: NavController) {
