...
- `strings-ot.xml` is InterTune's own. It is translated on
  [our Crowdin](https://crowdin.com/project/intertune), and a bot opens a pull request with
  whatever has been translated.
- `strings.xml` comes from OuterTune and is translated on
  [their Weblate](https://hosted.weblate.org/projects/outertune/). It reaches this repository
  through upstream merges. Translating it here would mean a merge and a sync writing over each
  other every time.

### Important

- Submit translations through Crowdin. If something cannot be done there, a pull request is fine
...
- Do not mess with translation files directly. `strings-ot.xml` is managed with Crowdin and
  `strings.xml` with upstream's Weblate.
